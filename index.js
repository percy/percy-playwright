...
async function slowScrollToBottom(page, scrollSleep = SCROLL_DEFAULT_SLEEP_TIME) {
  ...
  let scrollHeight = Math.min(await page.evaluate(scrollHeightCommand), CS_MAX_SCREENSHOT_LIMIT);
  const clientHeight = await page.evaluate('return document.documentElement.clientHeight');
  let current = 0;
...
const percySnapshot = async function(page, name, options) {
  ...
  try {
    ...
    log.info(`Taking DOM snapshot - v2"${name}"`);
